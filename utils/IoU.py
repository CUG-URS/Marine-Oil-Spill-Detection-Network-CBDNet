...
name_truth = '/data/nancy/oil/datasets/ours_oil_spill/test/palsar/gt/'
name_pred = '/data/nancy/oil/submits/scse_newpalsar_hyjoint_loss/'
mylog = open('/data/nancy/oil/utils/scse_newpalsar_hyjoint_loss2'+'.log','w')

def compute_iou1(img_truth,img_pred):
    # ytrue, ypred is a flatten vector
    im_truth = cv2.imread(img_truth, 0)
    #R,G,B = im_truth.split() 
    y_true = np.asarray(im_truth)
    #im_truth = Image.open(im_truth)
    #R,G,B = im_truth.split() 
    #y_true = np.asarray(B)
